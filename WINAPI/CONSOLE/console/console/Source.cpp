...
void drawLine(const HANDLE & hConsole, const WCHAR fill[CELL_LEN], const WCHAR separators[3], const int selectedCol = -1);
...
void drawLine(const HANDLE & hConsole, const WCHAR fill[CELL_LEN], const WCHAR separators[3], const int selectedCol) {

    WriteConsole(hConsole, &separators[0], 1, NULL, NULL);
    for (int j = 0; j <= max_col; j++) {
        if (j == selectedCol) {
            SetConsoleTextAttribute(hConsole, (WORD)((4 << 4) | 7));
            WriteConsole(hConsole, fill, CELL_LEN, NULL, NULL);
...
void drawTable(const HANDLE & hConsole, const Selection & selectedItem) {
    ...
    WriteConsole(hConsole, L"Task: Draw table. Row count: 4; Column count 5. Background: white; Border: red.\n", 80, NULL, NULL);
